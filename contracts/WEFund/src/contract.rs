...
use crate::state::{
    save_projectstate, BackerState, Config, Milestone, ProjectState, ProjectStatus, TeamMember,
    VestingParameter, Vote, COMMUNITY, CONFIG, PROJECTSTATES, PROJECT_SEQ,
};

// use crate::market::{
//     Cw20HookMsg, EpochStateResponse, ExecuteMsg as AnchorMarket, QueryMsg as AnchorQuery,
// };
...
pub fn instantiate(
    ...
    PROJECT_SEQ.save(deps.storage, &Uint64::new(0))?;
...
pub fn try_releasemilestone(
    ...
    let release_amount = x.milestone_states[step].milestone_amount.u128() * UST;
...
pub fn remove_project(deps: DepsMut, _project_id: Uint64) -> Result<Response, ContractError> {
    let res = PROJECTSTATES.may_load(deps.storage, _project_id.u64());
    if res == Ok(None) {
        return Err(ContractError::NotRegisteredProject {});
    }
    PROJECTSTATES.remove(deps.storage, _project_id.u64());
    Ok(Response::new())
}
...
pub fn try_failproject(
    ...
    for backer in x.backer_states {
        ...
        backed_ust.amount = Uint128::new(
            backer.ust_amount.amount.u128() * release_amount
    ...
    PROJECTSTATES.update(deps.storage, _project_id.u64(), |op| match op {
        None => Err(ContractError::NotRegisteredProject {}),
        Some(mut project) => {
...
pub fn try_back2project(
    ...
    if communitybacker_needback == false && backer_needback == false {
        ...
        if config.vesting_contract != "".to_string() && x.token_addr != "".to_string() {
            ...
            token_amount =
                token_amount * Uint128::new((10 as u128).pow(token_info.decimals as u32)); //for decimals
    ...
    let x = PROJECTSTATES.load(deps.storage, project_id.u64())?;
    if config.vesting_contract != "".to_string() && x.token_addr != "".to_string() {
